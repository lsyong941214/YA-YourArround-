...
- **밸런스 게임(주변인 테스트) 진행 중 앱 종료 후 이어하기**: `BlndGameScreen.tsx`는 카드
  ...
  진행 단계를 복원하는 구조라 설계상으로는 이미 이어하기가 되어야 한다. `blind_test_picks.card_idx`
  컬럼 누락 버그를 고친 `supabase/alter_blnd_picks.sql`을 실제 프로젝트에 반영했으니, 이제
  실제로 이어하기가 되는지 재현 테스트가 필요하다. 재현된다면 홈 화면에 "진행중인 매칭"으로
  바로 돌아갈 진입점이 부족한 게 원인일 수 있음.
