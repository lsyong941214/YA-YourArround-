...
## TODO (다음 작업 예정)
- **매칭 진행 상태를 하단 탭바에 반영**: 연결 요청/주변인 테스트를 신청하면, 신청한 본인의
  홈 하단 탭바 "매칭" 아이콘이 활성(하트 채워짐) 상태로 바뀌고 옆에 작은 숫자로 진행중(미완료)
  매칭 건수를 표시한다. 지금은 `HomeScreen.tsx`의 하단 탭바 배지(`prop_val`)가 "내가 받은" 미확인
  제안 수만 반영하고 있고, "내가 보낸" 진행중 건수(`sent_prog_val`)는 상단 카드 텍스트에만 쓰이고
  탭바 하트/배지에는 반영되지 않는다. `matc_store`/`blnd_store`의 진행중 건수 집계 함수를
  합쳐서 탭바에 노출하는 작업이 필요하다.
  - 같은 맥락으로, 보낸 요청이 수락되면 메인 화면 상단 "내 역할" 카드(주민 역할 표기 바로
    아래)의 "매칭 현황" 건수(`HomeScreen.tsx`의 `matc_val`/`sent_prog_val`)도 함께 갱신되어야
    한다. 지금 `sent_prog_cnt()`는 연결 요청(`matc_store`)만 세고 주변인 테스트(`blnd_store`)
    진행중 건수는 포함하지 않고 있어, 주변인 테스트가 수락돼도 이 숫자에 반영되지 않는다.
- **하단 탭바를 모든 화면에서 고정**: 지금은 `HomeScreen.tsx`에만 하단 탭바
  (`<nav className="fixed ...">`)가 구현돼 있고, 다른 라우트(매칭/연락처/마이페이지 등)로
  이동하면 탭바가 사라진다. 공용 레이아웃(`src/app/layout.tsx` 또는 별도 shell 컴포넌트)으로
  빼서 로그인 이후 모든 화면에 항상 고정되도록 개선한다.
- **밸런스 게임(주변인 테스트) 진행 중 앱 종료 후 이어하기**: `BlndGameScreen.tsx`는 카드
  선택마다 `submit_pick()`으로 `blind_test_picks`에 저장하고, 재진입 시 저장된 픽 개수로
  진행 단계를 복원하는 구조라 설계상으로는 이미 이어하기가 되어야 한다. 다만 최근까지
  `blind_test_picks.card_idx` 컬럼 누락 버그(`supabase/alter_blnd_picks.sql`로 수정)로 픽 저장
  자체가 실패했을 수 있어, 그 수정 반영 후에도 재현되는지 다시 확인이 필요하다. 재현된다면
  홈 화면에 "진행중인 매칭"으로 바로 돌아갈 진입점이 부족한 게 원인일 수 있음(위 탭바 배지
  항목과 연관).
