...
- **주변인 테스트 진행 중 종료/거절**: 요청을 받은 주민(`memb`) 쪽에서 게임 화면
  (`BlndGameScreen.tsx`) 헤더의 "종료하기" 버튼으로 진행 중인 테스트를 끝낼 수 있다 - 누르면
  기존 결과 화면의 종료 로직과 같은 `blnd_submit_actn` RPC(`submit_actn(blnd_id, "end")`)를
  호출해 상태를 `rjct`로 바꾸고 홈으로 돌아간다. 상대방(요청자) 화면은 Realtime으로 이를 받아
  "상대방이 더 이상의 진행을 원치 않는 것 같습니다." 팝업을 띄우고, "확인"을 누르면 홈으로
  이동한다. 게임 진행 중 이 전환이 `BlndReviewScreen`의 정적 안내 화면으로 잘못 되돌아가지
  않도록(팝업이 뜨기 전에 화면이 바뀌어버리는 것 방지), 게임 화면에 한 번 들어간 뒤로는
  상태와 무관하게 계속 게임 화면 쪽에서 전환을 처리하게 했다 - 두 계정으로 동시에 접속해
  실제로 팝업이 뜨고 라우팅이 되는지 확인이 필요하다.
