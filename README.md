...
- **서비스 출시 체크리스트 대응** (2026-09-18, 앱인토스 개발자센터 체크리스트 기준) —
  ...
  - ✅ 연령 검증(만 19세 미만 가입 차단, 온보딩), 금칙어(불법 광고 의심 문구) 필터링(소개글/
    ...
    숨김) 구현 완료 — DB는 `supabase/alter_rept_blck.sql` 실행 필요(위 "Supabase 설정" 참고)
  - ❌ 남은 항목: 계정 영구 정지 정책, 신고 24시간 대응 관리자 화면, AI 기반 이미지 검증,
    수사기관 협조 체계, 결제 환불 절차 — 세부는 LAUNCH_CHECKLIST.md 참고
