 ...
 * toss_auth.ts
 * 토스 로그인 연동
 * - TossAuth.login()으로 authorizationCode/referrer를 받는다(클라이언트에서는 이것만 받는다).
 * - authorizationCode를 실제 사용자 식별자로 교환하는 과정은 토스 파트너 API가 mTLS
 *   클라이언트 인증서를 요구해서 반드시 서버(Supabase Edge Function `toss-login`)에서만
 *   한다 - 클라이언트는 절대 하지 않는다.
...
export async function toss_lgin(): Promise<{ ok_flag: boolean; err_msg?: string }> {
  ...
  let referrer: string;
  try {
    ({ authorizationCode, referrer } = await TossAuth.login());
  } catch {
